...
export function BranchCreateDialog({
  ...
  const fetcher = useFetcher<BranchCreateActionData>({ key: "branch-create" });
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>요가원 등록</DialogTitle>
          <DialogDescription>새 요가원 정보를 입력해 주세요.</DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <BranchFormFields disabled={isSubmitting} idPrefix="branch-create" />
          <Button
            type="submit"
            className="md:col-span-2 md:justify-self-end"
            disabled={isSubmitting}
          >
            {isSubmitting ? "등록 중..." : "요가원 등록"}
          </Button>
        </fetcher.Form>
      </DialogContent>
    </Dialog>
